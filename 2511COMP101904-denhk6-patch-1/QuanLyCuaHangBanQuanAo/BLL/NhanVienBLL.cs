...
MatKhau = tkDTO.MatKhau, // Cần mã hóa mật khẩu ở đây
MaNhanVien = nvDAL.MaNV, // 2. Lấy MaNV vừa thêm
MaQuyen = tkDTO.MaQuyen
};
...
{
// 1. Cập nhật NhanVien
DAL.NhanVien nvDAL = db.NhanViens.Single(nv => nv.MaNV ==
nvDTO.MaNV);
...
// 2. Cập nhật TaiKhoan
DAL.TaiKhoan tkDAL = db.TaiKhoans.Single(tk => tk.MaNhanVien ==
nvDTO.MaNV);
...
{
tkDAL.MatKhau = tkDTO.MatKhau; // Cần mã hóa
}
db.SubmitChanges();
transaction.Commit();
}
